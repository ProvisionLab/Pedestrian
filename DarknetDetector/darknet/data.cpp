...
const char **find_replace_paths(const char **paths, int n, const char *find, const char *replace)
{
    const char **replace_paths = (const char**)calloc(n, sizeof(char*));
    int i;
    for(i = 0; i < n; ++i){
        const char *replaced = find_replace(paths[i], find, replace);
        replace_paths[i] = copy_string((char*)replaced);
    }
...
void *load_thread(void *ptr)
{
    ...
    if (a.type == OLD_CLASSIFICATION_DATA){
        ...
        *a.d = load_data_writing((const char**)a.paths, a.n, a.m, a.w, a.h, a.out_w, a.out_h);
...
//{
//    pthread_t thread;
//    struct load_args *ptr =(float*) calloc(1, sizeof(struct load_args));
//    *ptr = args;
//    if(pthread_create(&thread, 0, load_thread, ptr)) error("Thread creation failed");
//    return thread;
//}

data load_data_writing(const char **paths, int n, int m, int w, int h, int out_w, int out_h)
{
    if(m) paths = (const char**)get_random_paths((char**)paths, n, m);
    const char **replace_paths = find_replace_paths(paths, n, ".png", "-label.png");
    data d = {};
    d.shallow = 0;
    d.X = load_image_paths((char**)paths, n, w, h);
    d.y = load_image_paths_gray((char**)replace_paths, n, out_w, out_h);
    if(m) free(paths);
    int i;
    for(i = 0; i < n; ++i) free((void*)replace_paths[i]);
    free(replace_paths);
    return d;
}
