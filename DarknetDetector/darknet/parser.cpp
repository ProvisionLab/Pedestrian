...
void parse_data(const char *data, float *a, int n)
{
    int i;
    if(!data) return;
    char *curr =(char*) data;
...
deconvolutional_layer parse_deconvolutional(list *options, size_params params)
{
    ...
    const char *weights = option_find_str(options, "weights", 0);
    const char *biases = option_find_str(options, "biases", 0);
...
{
    list *sections = read_cfg((char*)filename);
...
{
    load_weights_upto(net, (char*) filename, net->n);
}
